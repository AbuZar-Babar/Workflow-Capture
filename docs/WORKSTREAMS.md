...
Statuses below are the initial assignment state for Task 0. `READY` means the task may be assigned now; `BLOCKED` means one or more task dependencies must be integrated first.
...
| Task | Dependencies | Initial status |
|---|---|---|
| Task 0 — Establish the agent workboard | None | READY |
| Task 1 — Secure dashboard local mode | None | READY |
| Task 2 — Stabilize repeated-item execution | None | READY |
| Task 3 — Isolate browser-backed tests | None | READY |
| Task 4 — Implement a reusable filter evaluator | None; shared filter contract must be frozen | READY |
| Task 5 — Unify discovery, review, filtering, and execution UI | Shared filter contract; coordinate with Task 6 response contract | READY |
| Task 6 — Integrate generic filters in API and runtime | Tasks 1, 2, 4; Task 5 may proceed in parallel | BLOCKED |
...
| Task 10 — Dashboard lifecycle correctness | Task 5 | BLOCKED |
